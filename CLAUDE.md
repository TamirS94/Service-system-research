...
8. **No-chosen choice sets — two causes.** Final table has ~3,197 visible no-chosen sets (more are fully dropped and invisible). Cause A = **proactive agent openers**: the chosen session has no pending visitor turn (no type=1 before the choice moment — silent abandonment or a customer who replies only later); ~5,400 sets (~0.3%), *not* yet filtered — open decision whether Stage 2 should emit a choice set for an agent reply that isn't answering a waiting customer. Cause B = the same-second tie bug (#7, now fixed).

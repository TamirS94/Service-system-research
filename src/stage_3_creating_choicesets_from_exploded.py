...
def main(merged_df: pd.DataFrame, df_exploded: pd.DataFrame, today_str: str):
    ...
    for i, row in enumerate(df_exploded.reset_index().itertuples(index=False)):
        ...
        # The reference every alternative's waiting_time / chosen_time is measured TO.
        # Always the choice moment T, including on the Option-3 branch below.
        choice_time = row.time
        # Where the waiting clock STARTS. None = the default (first pending message);
        # the Option-3 branch overrides it with the genuine reply time.
        waiting_start = None
        ...
        if consecutive_type1.empty:
            ...
            # Only the TURN is borrowed — waiting_time is still measured to this choice
            # moment, so the chosen row stays commensurable with the other alternatives.
            ...
            if row_flag == 1 and row.chosen == 1 and not type1_past.empty:
                ...
                # choice_time stays at row.time (the re-engagement moment), so chosen_time
                # is identical across the whole set and every alternative is read off one
                # clock. What differs is where that clock STARTS for this row:
                #   normal alternative  -> first pending visitor message (customer has an
                #                          unanswered turn; the wait is the customer's)
                #   Option-3 chosen row -> genuine_reply_time (the turn was already
                #                          answered; nothing is pending, so the interval
                #                          that matters is how long the session has sat
                #                          untouched since the agent last dealt with it)
                # Both are "time this session has been in its current state at T".
                waiting_start = genuine_reply_time
                logger.info(
                    f"flag=1 re-engagement: reused genuine turn (answered at "
                    f"{genuine_reply_time}); waiting_time = {choice_time} - {genuine_reply_time}"
                )
            else:
        ...
        if waiting_start is None:
            waiting_start = first_end_time
        waiting_time = choice_time - waiting_start
        ...
        logger.info(
            ...
            f"n_messages={aggregated_row['n_messages'].iloc[0]}, waiting_time={waiting_time}"
        )
